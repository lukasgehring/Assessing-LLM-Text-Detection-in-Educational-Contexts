...
def prompt_mode_comparison():
    ...
    for detector, sub_df in df.groupby("detector"):
        for mode in sub_df.prompt_mode.unique():
            if mode == "Human":
                continue
            sub_sub_df = sub_df[sub_df["prompt_mode"].isin(["Human", mode])]
            fpr, tpr, _ = get_roc_curve(sub_sub_df)
    ...
    desired_order = ["Improved-\nHuman", "Rewrite-\nHuman", "Summary", "Task+\nSummary", "Task", "Rewrite-LLM",
                     "Dipper"]

    sns.barplot(df, x='mode', y='roc_auc', hue='detector', order=desired_order, legend=False)
    plt.show()
