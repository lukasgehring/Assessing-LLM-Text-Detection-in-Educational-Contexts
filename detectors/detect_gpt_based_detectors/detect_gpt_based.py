...
def perturb_data(data, mask_model, mask_tokenizer, args):
    ...
    if args.use_detector_cache:
        ...
        hash_key_list = ['buffer_size', 'cut_sentences', 'dataset', 'mask_filling_model_name','mask_top_p','max_num_attempts', 'max_words', 'n_perturbation', 'pct_words_masked', 'span_length', 'seed','n_samples']
    ...
    if cached_human_data:
        for idx, result in enumerate(cached_human_data[0]['raw_results']):
            for k in ['original', 'sampled', 'perturbed_sampled', 'sampled_ll', 'all_perturbed_sampled_ll', 'perturbed_sampled_ll', 'perturbed_sampled_ll_std']:
                result.pop(k, None)
    ...
    # loading llm data if exist
        ...
        for idx, result in enumerate(cached_llm_data[0]['raw_results']):
            for k in ['sampled', 'original', 'perturbed_original', 'original_ll', 'all_perturbed_original_ll', 'perturbed_original_ll', 'perturbed_original_ll_std']:
                result.pop(k, None)
    ...
    if not human_loaded:
        ...
        logger.info(f"Finished generation of {len(p_original_text)} perturbations of the human-written texts. ({time.time() - start:.2f}s)")
