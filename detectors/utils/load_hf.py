...
def hf_load_pretrained_llm(name, model_kwargs=None, tokenizer_kwargs=None, model_class=transformers.AutoModelForCausalLM, tokenizer_class=transformers.AutoTokenizer, cache_dir='.resources', device_map="auto"):
    ...
    logger.success(f"Model {name} loaded {f'with args: {model_kwargs}' if model_kwargs is not None else ''}. ({time.time() - start:.2f}s)")
    ...
    logger.success(f"Tokenizer {name} loaded {f'with args: {tokenizer_kwargs}' if tokenizer_kwargs is not None else ''}. ({time.time() - start:.2f}s)")
