...
def get_predictions(data, enc, best_features, model, mu, sigma, trigram_model, args):
    ...
    for text in tqdm(data.answer):
        ...
        features = (np.array(t_features + exp_features) - mu) / sigma
        predicitons.append(model.predict_proba(features.reshape(-1, 1).T)[:, 1])
...
def run(data, args):
    ...
    if cached_human_data is None:
        ...
        logger.info(
            f"Finished computation of {len(human_predictions)} human-written texts. ({time.time() - start:.2f}s)")
    ...
    if cached_llm_data is None:
        ...
        logger.info(
            f"Finished computation of {len(human_predictions)} LLM-generated texts. ({time.time() - start:.2f}s)")
